...
import (
	...
	metamachv1 "k8s.io/apimachinery/pkg/apis/meta/v1"
...
const cRDName = crv1.AppFolderResourcePlural + "." + crv1.GroupName
...
func CreateCustomResourceDefinition(clientset apiextensionsclient.Interface) (*apiextensionsv1beta1.CustomResourceDefinition, error) {
	crd := &apiextensionsv1beta1.CustomResourceDefinition{
		ObjectMeta: metamachv1.ObjectMeta{
			Name: cRDName,
		},
		Spec: apiextensionsv1beta1.CustomResourceDefinitionSpec{
			...
			Names: apiextensionsv1beta1.CustomResourceDefinitionNames{
				Plural: crv1.AppFolderResourcePlural,
				Kind:   reflect.TypeOf(crv1.AppFolder{}).Name(),
			},
		},
	}
	...
	err = wait.Poll(500*time.Millisecond, 60*time.Second, func() (bool, error) {
		crd, err = clientset.ApiextensionsV1beta1().CustomResourceDefinitions().Get(cRDName, metamachv1.GetOptions{})
		if err != nil {
			return false, err
		}
...
func WaitForExampleInstanceProcessed(exampleClient *rest.RESTClient, name string) error {
	return wait.Poll(100*time.Millisecond, 10*time.Second, func() (bool, error) {
		var example crv1.AppFolder
		err := exampleClient.Get().
			Resource(crv1.AppFolderResourcePlural).
			Namespace(apiv1.NamespaceDefault).
			Name(name).
			Do().Into(&example)

		if err == nil && example.Status.State == crv1.AppFolderStateProcessed {
			return true, nil
		}
